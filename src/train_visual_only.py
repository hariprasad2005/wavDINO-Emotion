"""Train a visual-only emotion classifier from precomputed DINOv2 embeddings.

Inputs: train/val .npy files produced by extract_visual.py (list of dicts with keys path,label,embedding).
Outputs: best checkpoint (.pt) + meta (.json) + log (.txt).
"""
...
from train_fusion import LABELS, VISUAL_DIM, accuracy, load_embeddings, macro_f1
...
def build_items(path: Path) -> List[Dict[str, object]]:
    # Load list-of-dicts from .npy produced by extract_visual.py
...
class EmbeddingClassifier(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, dropout: float, num_classes: int):
        super().__init__()
        # Lightweight MLP classifier over visual embeddings
...
def eval_epoch(model: nn.Module, loader: DataLoader, device: str, num_classes: int) -> Dict[str, float]:
    # Evaluate loss/acc/macro-F1 for one pass over the loader
    model.eval()
...
def train(train_loader: DataLoader, val_loader: DataLoader, device: str, epochs: int, lr: float, model_args: Dict[str, object], log_path: Path, model_path: Path) -> Dict[str, float]:
    # Train loop with best checkpoint tracked by validation accuracy
...
def build_loader(path: Path, label_to_idx: Dict[str, int], batch_size: int, shuffle: bool) -> DataLoader:
    # Convenience wrapper to create DataLoader with consistent labels
    ds = VisualDataset(build_items(path), label_to_idx)
    return DataLoader(ds, batch_size=batch_size, shuffle=shuffle)


def derive_labels(paths: Sequence[Path]) -> List[str]:
    # Infer label set present in the provided splits, ordered by canonical LABELS
    labels = set()
    for p in paths:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train visual-only emotion classifier")
    ...
    parser.add_argument("--log_path", type=Path, default=Path("./logs/visual_training_log.txt"))
    parser.add_argument("--model_path", type=Path, default=Path("./models/visual_classifier.pt"))
